...
| [`nightly.yml`](.github/workflows/nightly.yml) | Every push to `main` | Builds all desktop platforms **and the Android APK** and publishes them to a single rolling **`nightly`** pre-release, recreated each run so it always tracks the latest commit. |
| [`release.yml`](.github/workflows/release.yml) | Push of a `v*` tag (or manual dispatch) | Builds all desktop platforms **and the Android APK** and creates a **stable, versioned** GitHub Release (as a draft to review before publishing). |
...
| [`android.yml`](.github/workflows/android.yml) | Pull requests + manual dispatch | PR-gating Android build: builds + verifies the APK (manifest + dex) and uploads it as a workflow artifact. (Nightly/release attach the APK to the GitHub Release.) |

The Android build is shared via a composite action,
[`.github/actions/android-build`](.github/actions/android-build/action.yml).
